  ...
  const handleDownload = async (format: "csv" | "json") => {
    ...
    try {
      ...
      if (format === "csv") {
        ...
        link.setAttribute("download", `export_vegetation_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        const link = document.createElement("a");
        link.href = downloadUrl;
        link.setAttribute("download", `export_vegetation_${new Date().toISOString().split('T')[0]}.json`);
        document.body.appendChild(link);
        link.click();
        link.remove();
        window.URL.revokeObjectURL(downloadUrl);
...
function VegetationApiBuilder() {
  ...
  const baseUrl = `${NEXT_PUBLIC_BACKEND_URL}/api/datastore/vegetation/`;
  ...
  const params = useMemo(() => {
    ...
    return p;
  }, [start, end, geocode, uf, isValidUf, collection, attribute, page, perPage]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium opacity-70">collection</label>
          <input
            type="text"
            placeholder="e.g. modis"
            value={collection}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium opacity-70">attribute</label>
          <input
            type="text"
...
export function VegetationView({ config }: { config: EndpointDetails }) {
  const now = new Date();
  const oneYearAgo = new Date();
  oneYearAgo.setDate(now.getDate() - 365);
  const formatDateISO = (date: Date) => date.toISOString().split('T')[0];
  ...
  return (
    <EndpointLayout
      ...
      apiBuilder={<VegetationApiBuilder />}
      controls={
        <>
